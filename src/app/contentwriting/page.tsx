'use client';
import React, { useEffect, useRef, useState } from 'react';
import { FileText, PenTool, Search, BookOpen, Mail, Newspaper, Check, ArrowRight, Target, TrendingUp } from 'lucide-react';

const ContentWritingService = () => {
    const [isVisible, setIsVisible] = useState({});
    const observerRef = useRef(null);

    useEffect(() => {
        observerRef.current = new IntersectionObserver(
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        setIsVisible((prev) => ({
                            ...prev,
                            [entry.target.dataset.id]: true,
                        }));
                    }
                });
            },
            { threshold: 0.1, rootMargin: '0px 0px -50px 0px' }
        );

        const elements = document.querySelectorAll('[data-animate]');
        elements.forEach((el) => observerRef.current.observe(el));

        return () => observerRef.current?.disconnect();
    }, []);

    const getAnimationClass = (id) => {
        return isVisible[id]
            ? 'opacity-100 translate-y-0 transition-all duration-700 ease-out'
            : 'opacity-0 translate-y-10';
    };

    return (
        <div className="min-h-screen bg-white">
            {/* Hero Section */}
            <section
                className="bg-gradient-to-br from-[#002346] to-[#003a6b] text-white py-12 md:py-20 px-4 md:px-8"
                data-animate
                data-id="hero"
            >
                <div className={`max-w-7xl mx-auto ${getAnimationClass('hero')}`}>
                    <div className="text-center mb-8 md:mb-12 pt-11">
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            AI-DRIVEN CONTENT WRITING SERVICES
                        </p>
                        <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold mb-4 md:mb-6 leading-tight">
                            Content Built for Visibility, <br className="hidden md:block" />
                            <span className="text-[#FFA22B]">Authority, and Conversions</span>
                        </h1>
                        <p className="text-base md:text-lg lg:text-xl text-gray-300 max-w-3xl mx-auto mb-6 md:mb-8 px-4">
                            Empire Offshore provides professionally written content tailored to business goals, search intent, and audience psychology. The focus is not just writing words, but creating content systems that attract traffic, build trust, and convert readers into customers.
                        </p>
                        <div className="flex flex-col sm:flex-row gap-4 justify-center px-4">
                            <a href="/#contact" className="bg-[#FFA22B] text-[#002346] px-6 md:px-8 py-3 md:py-4 rounded-lg font-semibold hover:bg-[#ff8f00] transition text-sm md:text-base shadow-xl hover:shadow-2xl transform hover:scale-105 inline-block text-center">
                                Schedule a Call
                            </a>
                            <a href="/#portfolio" className="bg-transparent border-2 border-white text-white px-6 md:px-8 py-3 md:py-4 rounded-lg font-semibold hover:bg-white hover:text-[#002346] transition text-sm md:text-base inline-block text-center">
                                View Writing Samples
                            </a>
                        </div>
                    </div>
                </div>
            </section>

            {/* What We Deliver */}
            <section className="py-12 md:py-20 px-4 md:px-8 bg-white">
                <div className="max-w-7xl mx-auto">
                    <div
                        className={`text-center mb-8 md:mb-16 ${getAnimationClass('deliver-title')}`}
                        data-animate
                        data-id="deliver-title"
                    >
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            CONTENT TYPES INCLUDED
                        </p>
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#002346] mb-3 md:mb-4">
                            What We <span className="text-[#FFA22B]">Deliver</span>
                        </h2>
                        <p className="text-sm md:text-lg text-gray-600 max-w-2xl mx-auto">
                            Professional content writing services for every business need
                        </p>
                    </div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        {[
                            {
                                icon: <FileText className="w-10 h-10 md:w-12 md:h-12" />,
                                title: "SEO Blog Articles",
                                description: "Content optimized for search engines that drives organic traffic and engagement",
                                items: ["Keyword research", "Search intent mapping", "SEO optimization"]
                            },
                            {
                                icon: <PenTool className="w-10 h-10 md:w-12 md:h-12" />,
                                title: "Website Pages and Landing Pages",
                                description: "Compelling web content designed to convert visitors into customers",
                                items: ["Homepage copy", "Service pages", "Landing pages"]
                            },
                            {
                                icon: <Target className="w-10 h-10 md:w-12 md:h-12" />,
                                title: "Sales Copy and Funnels",
                                description: "Persuasive copy that guides prospects through your sales funnel",
                                items: ["Sales pages", "Funnel sequences", "CTA optimization"]
                            },
                            {
                                icon: <Mail className="w-10 h-10 md:w-12 md:h-12" />,
                                title: "Email Marketing Sequences",
                                description: "Strategic email content that nurtures leads and drives conversions",
                                items: ["Welcome sequences", "Nurture campaigns", "Promotional emails"]
                            },
                            {
                                icon: <BookOpen className="w-10 h-10 md:w-12 md:h-12" />,
                                title: "Product and Service Descriptions",
                                description: "Clear, benefit-focused descriptions that showcase value and drive purchases",
                                items: ["Product features", "Benefit highlights", "Technical specs"]
                            },
                            {
                                icon: <Newspaper className="w-10 h-10 md:w-12 md:h-12" />,
                                title: "Thought Leadership Content",
                                description: "Authority-building content that establishes expertise and builds trust",
                                items: ["Industry insights", "Expert opinions", "Trend analysis"]
                            }
                        ].map((service, index) => (
                            <div
                                key={index}
                                className={`bg-white border border-gray-200 rounded-xl p-6 md:p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 ${getAnimationClass(`service-${index}`)}`}
                                data-animate
                                data-id={`service-${index}`}
                            >
                                <div className="text-[#FFA22B] mb-4 md:mb-6">{service.icon}</div>
                                <h3 className="text-lg md:text-xl font-bold text-[#002346] mb-2 md:mb-3">{service.title}</h3>
                                <p className="text-sm md:text-base text-gray-600 mb-4">{service.description}</p>
                                <ul className="space-y-2">
                                    {service.items.map((item, i) => (
                                        <li key={i} className="flex items-center text-xs md:text-sm text-gray-700">
                                            <Check className="w-4 h-4 md:w-5 md:h-5 text-[#FFA22B] mr-2 flex-shrink-0" />
                                            {item}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* Technology Stack */}
            <section className="py-12 md:py-20 px-4 md:px-8 bg-gray-50">
                <div className="max-w-7xl mx-auto">
                    <div
                        className={`text-center mb-8 md:mb-16 ${getAnimationClass('tech-title')}`}
                        data-animate
                        data-id="tech-title"
                    >
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            TOOLS & EXPERTISE
                        </p>
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#002346] mb-3 md:mb-4">
                            Our Content <span className="text-[#FFA22B]">Tech Stack</span>
                        </h2>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                        {[
                            { name: "WordPress", category: "CMS" },
                            { name: "Grammarly", category: "Editing" },
                            { name: "Hemingway", category: "Readability" },
                            { name: "Surfer SEO", category: "SEO Tool" },
                            { name: "SEMrush", category: "Research" },
                            { name: "Ahrefs", category: "SEO Analysis" },
                            { name: "Google Docs", category: "Collaboration" },
                            { name: "Copyscape", category: "Plagiarism Check" },
                            { name: "Canva", category: "Visual Content" },
                            { name: "Notion", category: "Planning" },
                            { name: "HubSpot", category: "Content Hub" },
                            { name: "Yoast SEO", category: "Optimization" }
                        ].map((tech, index) => (
                            <div
                                key={index}
                                className={`bg-white rounded-lg p-4 md:p-6 text-center hover:shadow-lg transition-all duration-300 hover:-translate-y-1 ${getAnimationClass(`tech-${index}`)}`}
                                data-animate
                                data-id={`tech-${index}`}
                            >
                                <div className="text-xl md:text-2xl font-bold text-[#002346] mb-1 md:mb-2">{tech.name}</div>
                                <div className="text-xs md:text-sm text-[#FFA22B] font-semibold">{tech.category}</div>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* Development Process */}
            <section className="py-12 md:py-20 px-4 md:px-8 bg-white">
                <div className="max-w-7xl mx-auto">
                    <div
                        className={`text-center mb-8 md:mb-16 ${getAnimationClass('process-title')}`}
                        data-animate
                        data-id="process-title"
                    >
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            HOW WE WORK
                        </p>
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#002346] mb-3 md:mb-4">
                            Our Content Creation <span className="text-[#FFA22B]">Process</span>
                        </h2>
                    </div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-6 md:gap-8">
                        {[
                            { num: "01", title: "Discovery", desc: "Understanding your brand voice, audience, and content goals", color: "bg-[#FFA22B]" },
                            { num: "02", title: "Strategy", desc: "Developing content calendar and keyword research", color: "bg-[#002346]" },
                            { num: "03", title: "Research", desc: "In-depth topic research and competitor analysis", color: "bg-[#FFA22B]" },
                            { num: "04", title: "Writing", desc: "Creating engaging, SEO-optimized content drafts", color: "bg-[#002346]" },
                            { num: "05", title: "Review", desc: "Editing, proofreading, and quality assurance", color: "bg-[#FFA22B]" }
                        ].map((step, index) => (
                            <div
                                key={index}
                                className={`text-center ${getAnimationClass(`process-${index}`)}`}
                                data-animate
                                data-id={`process-${index}`}
                            >
                                <div className={`${step.color} text-white w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center text-xl md:text-2xl font-bold mx-auto mb-3 md:mb-4 shadow-lg`}>
                                    {step.num}
                                </div>
                                <h3 className="text-lg md:text-xl font-bold text-[#002346] mb-2">{step.title}</h3>
                                <p className="text-xs md:text-sm text-gray-600">{step.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* Why Choose Us */}
            <section className="py-12 md:py-20 px-4 md:px-8 bg-[#002346] text-white">
                <div className="max-w-7xl mx-auto">
                    <div
                        className={`text-center mb-8 md:mb-16 ${getAnimationClass('why-title')}`}
                        data-animate
                        data-id="why-title"
                    >
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            KEY BENEFITS
                        </p>
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold mb-3 md:mb-4">
                            Why Choose <span className="text-[#FFA22B]">Empire Offshore?</span>
                        </h2>
                    </div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        {[
                            { icon: <Search className="w-10 h-10 md:w-12 md:h-12" />, title: "Improves Search Rankings", desc: "SEO-optimized content that ranks higher and drives organic traffic" },
                            { icon: <Target className="w-10 h-10 md:w-12 md:h-12" />, title: "Builds Brand Authority", desc: "Thought leadership content that establishes trust and credibility" },
                            { icon: <TrendingUp className="w-10 h-10 md:w-12 md:h-12" />, title: "Increases Engagement", desc: "Compelling content that reduces bounce rate and keeps visitors reading" },
                            { icon: <FileText className="w-10 h-10 md:w-12 md:h-12" />, title: "Converts to Customers", desc: "Persuasive copy that turns readers into leads and paying customers" },
                            { icon: <PenTool className="w-10 h-10 md:w-12 md:h-12" />, title: "Saves Time & Cost", desc: "Eliminates in-house hiring costs while maintaining high quality output" },
                            { icon: <Check className="w-10 h-10 md:w-12 md:h-12" />, title: "Long-term ROI", desc: "Content assets that generate value and traffic for years to come" }
                        ].map((item, index) => (
                            <div
                                key={index}
                                className={`bg-[#003a6b] rounded-xl p-6 md:p-8 hover:bg-[#004a7c] transition-all duration-300 ${getAnimationClass(`why-${index}`)}`}
                                data-animate
                                data-id={`why-${index}`}
                            >
                                <div className="text-[#FFA22B] mb-4 md:mb-6">{item.icon}</div>
                                <h3 className="text-lg md:text-xl font-bold mb-2 md:mb-3">{item.title}</h3>
                                <p className="text-sm md:text-base text-gray-300">{item.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* Industries We Serve */}
            <section className="py-12 md:py-20 px-4 md:px-8 bg-gray-50">
                <div className="max-w-7xl mx-auto">
                    <div
                        className={`text-center mb-8 md:mb-16 ${getAnimationClass('industries-title')}`}
                        data-animate
                        data-id="industries-title"
                    >
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            SERVICE VARIATIONS
                        </p>
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#002346] mb-3 md:mb-4">
                            Content <span className="text-[#FFA22B]">Solutions</span>
                        </h2>
                        <p className="text-sm md:text-lg text-gray-600 max-w-2xl mx-auto">
                            Specialized content strategies tailored to your business objectives
                        </p>
                    </div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {[
                            { title: "SEO Focused Content", desc: "Content optimized for organic traffic growth and search engine rankings" },
                            { title: "Conversion Copy", desc: "Sales pages and landing pages designed to convert visitors into customers" },
                            { title: "Brand Voice Content", desc: "Consistent messaging that aligns with your brand identity and values" },
                            { title: "Long Form Authority Blogs", desc: "In-depth articles that establish expertise and thought leadership" },
                            { title: "Localized Content", desc: "Regional market content tailored to specific geographic audiences" },
                            { title: "Technical Content", desc: "Complex topics simplified for your target audience understanding" },
                            { title: "E-commerce Content", desc: "Product descriptions and category pages that drive sales" },
                            { title: "Content Strategy", desc: "Complete content planning and editorial calendar management" }
                        ].map((industry, index) => (
                            <div
                                key={index}
                                className={`bg-white rounded-xl p-6 border-l-4 border-[#FFA22B] hover:shadow-xl transition-all duration-300 ${getAnimationClass(`industry-${index}`)}`}
                                data-animate
                                data-id={`industry-${index}`}
                            >
                                <h3 className="text-lg font-bold text-[#002346] mb-2">{industry.title}</h3>
                                <p className="text-sm text-gray-600">{industry.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* Key Features */}
            <section className="py-12 md:py-20 px-4 md:px-8 bg-white">
                <div className="max-w-7xl mx-auto">
                    <div
                        className={`text-center mb-8 md:mb-16 ${getAnimationClass('features-title')}`}
                        data-animate
                        data-id="features-title"
                    >
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            BUILT-IN EXCELLENCE
                        </p>
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#002346] mb-3 md:mb-4">
                            Essential <span className="text-[#FFA22B]">Features</span> in Every Piece
                        </h2>
                    </div>

                    <div className="grid md:grid-cols-2 gap-6">
                        {[
                            {
                                title: "SEO Optimization",
                                desc: "Every piece is crafted with search engines in mind for maximum visibility",
                                features: ["Keyword integration", "Meta descriptions", "Internal linking strategy"]
                            },
                            {
                                title: "Brand Voice Alignment",
                                desc: "Content that perfectly matches your brand's tone and messaging",
                                features: ["Style guide adherence", "Consistent terminology", "Brand personality"]
                            },
                            {
                                title: "Original & Plagiarism-Free",
                                desc: "100% unique content verified through multiple plagiarism checkers",
                                features: ["Copyscape verification", "Original research", "Proper citations"]
                            },
                            {
                                title: "Audience Psychology",
                                desc: "Content tailored to your target audience's needs, pain points, and desires",
                                features: ["Buyer persona research", "Pain point addressing", "Emotional triggers"]
                            },
                            {
                                title: "Search Intent Mapping",
                                desc: "Content structured to match what users are actually searching for",
                                features: ["Intent analysis", "Query optimization", "User journey mapping"]
                            },
                            {
                                title: "Conversion Focus",
                                desc: "Strategic content designed to turn readers into leads and customers",
                                features: ["Clear CTAs", "Benefit-driven copy", "Trust building elements"]
                            }
                        ].map((feature, index) => (
                            <div
                                key={index}
                                className={`bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl p-6 md:p-8 hover:shadow-xl transition-all duration-300 ${getAnimationClass(`feature-${index}`)}`}
                                data-animate
                                data-id={`feature-${index}`}
                            >
                                <h3 className="text-xl font-bold text-[#002346] mb-3">{feature.title}</h3>
                                <p className="text-gray-600 mb-4">{feature.desc}</p>
                                <ul className="space-y-2">
                                    {feature.features.map((item, i) => (
                                        <li key={i} className="flex items-center text-sm text-gray-700">
                                            <div className="w-2 h-2 bg-[#FFA22B] rounded-full mr-3"></div>
                                            {item}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* Project Timeline */}
            <section className="py-12 md:py-20 px-4 md:px-8 bg-[#002346] text-white">
                <div className="max-w-7xl mx-auto">
                    <div
                        className={`text-center mb-8 md:mb-16 ${getAnimationClass('timeline-title')}`}
                        data-animate
                        data-id="timeline-title"
                    >
                        <p className="text-[#FFA22B] text-xs md:text-sm font-semibold mb-2 md:mb-4 tracking-wider uppercase">
                            BUSINESS IMPACT
                        </p>
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold mb-3 md:mb-4">
                            Expected <span className="text-[#FFA22B]">Results</span>
                        </h2>
                    </div>

                    <div className="grid md:grid-cols-3 gap-6 md:gap-8">
                        {[
                            { type: "Month 1-3", time: "Foundation", scope: "Higher lead quality and improved organic visibility from optimized content" },
                            { type: "Month 3-6", time: "Growth Phase", scope: "Better organic visibility with consistent traffic increases and engagement" },
                            { type: "Month 6+", time: "ROI Phase", scope: "Long-term marketing ROI without ongoing ad spend dependency" }
                        ].map((timeline, index) => (
                            <div
                                key={index}
                                className={`bg-[#003a6b] rounded-xl p-6 md:p-8 text-center hover:bg-[#004a7c] transition-all duration-300 ${getAnimationClass(`timeline-${index}`)}`}
                                data-animate
                                data-id={`timeline-${index}`}
                            >
                                <div className="text-[#FFA22B] text-3xl md:text-4xl font-bold mb-3">{timeline.type}</div>
                                <h3 className="text-xl font-bold mb-3">{timeline.time}</h3>
                                <p className="text-gray-300 text-sm">{timeline.scope}</p>
                            </div>
                        ))}
                    </div>
                </div>
            </section>

            {/* CTA Section */}
            <section
                className="py-16 md:py-24 px-4 md:px-8 bg-[#FFA22B]"
                data-animate
                data-id="cta"
            >
                <div className={`max-w-4xl mx-auto text-center ${getAnimationClass('cta')}`}>
                    <h2 className="text-3xl md:text-5xl lg:text-6xl font-bold text-white mb-4 md:mb-6 leading-tight">
                        Ready to Scale Your Content Production?
                    </h2>
                    <p className="text-lg md:text-xl lg:text-2xl text-white mb-8 md:mb-10">
                        Let's discuss how our content systems can attract traffic, build trust, and convert customers
                    </p>
                    <a href="/#contact" className="bg-white text-[#002346] px-10 md:px-14 py-4 md:py-5 rounded-lg font-bold hover:bg-gray-100 transition text-base md:text-xl shadow-xl hover:shadow-2xl transform hover:scale-105 inline-flex items-center gap-3">
                        Schedule Your Consultation
                        <ArrowRight className="w-5 h-5 md:w-6 md:h-6" />
                    </a>
                </div>
            </section>
        </div>
    );
};

export default ContentWritingService;